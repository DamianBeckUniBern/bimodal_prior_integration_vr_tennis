...
#simple size to large for shapiro test
#shapiro.test(residuals(model6_fast)) 
...
for (i in 1:24) {
  ...
  path <- file.path("plots/individual_plots_day2_and_day3/fast", paste(as.character(i),"fast_plot.png"))
...
for (i in 1:24) {
  ...
  i_plot <- ggplot(individual_data, aes(x = ball_position, y = horizontal_difference)) +
    ...
    theme(legend.position = "none")
  path <- file.path("plots/individual_plots_day2_and_day3/moderate", paste(as.character(i),"moderate_plot_day2_and_day3.png"))
  ggsave(path, i_plot, width = 9, height = 6, units = "cm")
}
...
for (i in 1:24) {
  ...
  i_plot <- ggplot(individual_data, aes(x = ball_position, y = horizontal_difference)) +
    ...
    theme(legend.position = "none")
  path <- file.path("plots/individual_plots_day2_and_day3/slow", paste(as.character(i),"slow_plot.png"))
  ggsave(path, i_plot, width = 9, height = 6, units = "cm")
}
...
# Save the plot
ggsave(filename = "plots/all_subjects_day2_and_day3.png", plot = p, width = 8, height = 6)
...
# Save the plot
ggsave(filename = "plots/all_subjects_interaction_day2_and_day3.png", plot = p, width = 8, height = 6)
