...
# Import data ----
#---------------------------------------------------------------
data_all <- read.csv("data/output_day1.csv") 
View(data_all)
summary(data_all)
...
# Filter data ----
#---------------------------------------------------------------
#make a new dataset warmup with only 0-95 in trial numbers
warm_up <- data_all %>% group_by(trial_number) %>% filter(trial_number < 96)

#delete warm_up data from data_all (the first 96 trials of each subject)
data_all <- data_all %>% anti_join(warm_up)
...
# Save the plot
ggsave(filename = "plots/all_subjects_day1.png", plot = p, width = 8, height = 6)
...
# Save the plot
ggsave(filename = "plots/all_subjects_interaction_day1.png", plot = p, width = 8, height = 6)
